...
import org.librepilot.cloud.uavtalk.UAVTalkMessage;
import org.librepilot.cloud.uavtalk.UAVTalkObject;
import org.librepilot.cloud.uavtalk.UAVTalkObjectInstance;
...
public class FcLogfileWaiterThread extends FcWaiterThread {
    ...
    private void reportObjectCount(int i) {
        if (mEventListener != null) {
            mEventListener.reportObjectCount(i);
        }
        mState = i;
    }
    ...
    public void run() {
        ...
        InputStream fis = Main.class.getClassLoader().getResourceAsStream("logs/"+mFilename);
    ...
    private void reportRuntime(int ts) {
        if (mEventListener != null) {
            mEventListener.reportRuntime(ts);
        }
    }
